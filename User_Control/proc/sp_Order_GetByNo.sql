IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[sp_Order_GetByNo]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[sp_Order_GetByNo]
GO
create PROC [sp_Order_GetByNo](
	@No				varchar(40)
)
AS
BEGIN
